...
# ── Document (paper / deck) branch ───────────────────────────────────────────
# Documents share the TEXT collection with transcripts rather than getting one
# of their own: they are text, they are embedded by the same bge model at the
# same dimension, and the existing text branch then retrieves them with no
# change to its query path. A separate collection per kind would need a second
# query, a second threshold, and a merge - and would miss the assignment's
# actual point, which is one index serving every source kind.

def upsert_document_chunks(doc_id: str, vectors: np.ndarray,
                           payloads: list[dict[str, Any]]) -> None:
    """Paper/deck chunks into the text collection.

    Point ids are uuid5 of '<doc_id>:doc:<idx>' (see docparse.chunk_point_id) -
    deterministic, so a retried or redelivered run overwrites its own points
    instead of duplicating them, and distinct from both frame ids ('<id>:<n>')
    and transcript ids ('<id>:text:<n>').
    """
    from ..ingest.docparse import chunk_point_id

    points = [
        qm.PointStruct(id=chunk_point_id(doc_id, payload["idx"]),
...
def count_document_points(user_id: str, doc_id: str) -> int:
    """How many points this document actually has in the index, read back.

    This is the verification half of the ordering invariant: upsert succeeds ->
    VERIFY -> only then commit 'indexed'. `wait=True` on the upsert means the
    write was acknowledged, but acknowledged is not the same as visible, and
    marking a source indexed on the strength of a call that returned is how
    sources go missing after a crash.
    """
    try:
        result = client().count(
            collection_name=TEXT_COLLECTION,
            count_filter=_user_filter(user_id, doc_id),
            exact=True,
        )
