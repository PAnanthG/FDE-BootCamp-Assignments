"""
providers.py  -  one adaptor: OpenAI.
...
# macOS `say` uses the system default voice regardless of the text's language,
# which mispronounces Spanish when that default is an English voice. Map
# locale -> a native `say` voice so system TTS sounds right per language.
# Override with SYSTEM_TTS_VOICE_<LOCALE> (e.g. SYSTEM_TTS_VOICE_ES_ES=Paulina).
SYSTEM_TTS_VOICES = {
    "es-ES": "Mónica",
}


def _system_tts_args(text: str, locale: str | None) -> list[str]:
    cmd = os.getenv("SYSTEM_TTS_CMD", "say")
    voice = None
    if locale:
        env_key = f"SYSTEM_TTS_VOICE_{locale.upper().replace('-', '_')}"
        voice = os.getenv(env_key) or SYSTEM_TTS_VOICES.get(locale)
    return [cmd, "-v", voice, text] if voice else [cmd, text]
...
class Provider:
    ...
    def __init__(self, name: str | None = None):
        name = (name or os.getenv("PROVIDER", "openai")).lower()
    ...
    # --- TTS ---
    def synthesize(self, text: str, locale: str | None = None) -> bytes | None:
        """Return WAV bytes for `text`, or None if played directly by the OS."""
        if self.tts_backend == "system":
            subprocess.run(_system_tts_args(text, locale), check=False)
            return None
        speech_args = {
...
class MockProvider:
    ...
    loop without touching OpenAI. Enable with PROVIDER=mock.
    """
