...
def _as_jpeg(image: bytes) -> bytes:
    """Re-encode any Pillow-readable image to JPEG. Cheap no-op if already one."""
    from PIL import Image
...
DESCRIBE_SYSTEM = (
    "You describe a single slide image for a search index. Be concrete and "
    "specific: name what is depicted and transcribe visible text. Never say "
    "you cannot see the image, and never add commentary about the task."
)


def describe_image(image: bytes, prompt: str, cfg: LLMConfig) -> str:
    """One image in, a plain description out - for captioning deck slides.

    Deliberately NOT `answer()`: that one injects citation instructions and a
    `[n] @ timestamp` label, which belong to the video read path and would end
    up embedded in the caption text and then in the index. Same providers, same
    downscaling, different contract.
    """
    # _downscale returns its input untouched when the image is already small,
    # so a small PNG would come back as PNG and then be sent under a
    # data:image/jpeg header. Re-encode first: callers render slides, and a
    # renderer's natural output is PNG.
    image = _downscale(_as_jpeg(image))
    if cfg.provider == "anthropic":
        import anthropic
