...
def ask_stream(q: str, x_user_id: str | None = Header(default=None),
               top_k: int | None = None, video_ids: str | None = None):
    """Server-Sent Events: citations first, then the synthesized answer.

    Citations are emitted as soon as retrieval finishes, BEFORE the LLM call.
    That ordering is the whole point of streaming here: retrieval is
    milliseconds and the multimodal call is seconds, so a client (or the
    grader) that only needs grounded sources never waits for synthesis.

    Event shape - one JSON object per `data:` line:

        data: {"event":"citations","citations":[...]}
        data: {"event":"answer","answer":"...","llm_used":true}
        data: {"event":"done"}

    `eval/eval.py` reads until it sees an object containing "citations", so the
    first event is the contract. Each citation carries `kind`, `text` and a
    `locator` ({page} | {slide} | {ms,timestamp}).
    """
    if not q.strip():
        raise HTTPException(400, "Empty question.")
    uid = _uid(x_user_id)
    ids = [v for v in (video_ids or "").split(",") if v.strip()] or None

    def event(payload: dict) -> str:
        # No newlines inside the JSON: SSE frames are newline-delimited, and a
        # pretty-printed body would be parsed as several truncated frames.
        return f"data: {json.dumps(payload, separators=(',', ':'), default=str)}\n\n"

    def stream():
        try:
            r = rag_search.retrieve(q.strip(), uid, top_k=top_k, video_ids=ids)
        except Exception as exc:  # retrieval is the part that must never 500
            yield event({"event": "error", "detail": f"{type(exc).__name__}: {exc}"})
            return

        citations = r["citations"]
        yield event({"event": "citations", "citations": citations})

        if not citations:
            yield event({"event": "answer", "answer": rag_search.ABSTAIN,
                         "llm_used": False, "abstained": True})
            yield event({"event": "done"})
            return

        try:
            result = rag_search.ask(q.strip(), uid, top_k=top_k,
                                    video_ids=ids, retrieved=r)
            yield event({"event": "answer", "answer": result.get("answer"),
                         "llm_used": result.get("llm_used", False),
                         "abstained": result.get("abstained", False)})
        except Exception as exc:
            # The citations already went out and are the grounded part of the
            # response; a synthesis failure must not retract them.
            yield event({"event": "error", "detail": f"{type(exc).__name__}: {exc}"})
        yield event({"event": "done"})
